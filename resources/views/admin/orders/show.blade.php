                ...
                <div>
                    ...
                    <form action="{{ route('order.updateStatus', $order) }}" method="POST" class="inline">
                        ...
                        <select name="status" onchange="this.form.submit()"
                            class="ml-2 px-2 py-1 rounded border border-gray-300 focus:ring-2 focus:ring-green-600">
                ...
                <div>
                    ...
                    <div class="mt-1">
                        @if ($order->shippingAddress)
                            <p class="font-medium text-gray-800">{{ $order->shippingAddress->label }}</p>
                            <p class="text-sm text-gray-600">{{ $order->shippingAddress->full_address }}</p>
                        @else
                            <p class="text-sm text-gray-600">Alamat pengiriman tidak tersedia</p>
                        @endif
                    </div>
                </div>
